...
def sample_path(name, theta):
	...
	if name == "primitive tetragonal":
		...
		th = np.deg2rad(60)
		cell = np.array([[s, 0, 0], [np.cos(th) * s, np.sin(th) * s, 0], [0, 0, c]])
...
def run():
	...
	print("\tfcc")
	data['fcc'].append(auguste.calculate_vector([[0, 1, 1], [1, 0, 1], [1, 1, 0]]))
	print("\tbcc")
	data['bcc'].append(auguste.calculate_vector([[-1, 1, 1], [1, -1, 1], [1, 1, -1]]))
